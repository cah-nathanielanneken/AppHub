...
AppHubControllers.controller('AppViewCntrl', ['$scope', 'App', '$location', '$routeParams', '$window','LogedInUser','AppComment',
                                              function($scope, App, $location, $routeParams, $window, LogedInUser, AppComment) {
	$scope.newComment = {};
	...
	$scope.createNewComment = function() {
		$scope.newComment.userName = LogedInUser.user.userName;
		$scope.newComment.appId = $scope.app.appID;
		$scope.newComment.approvalComment = false;
		$scope.newComment.commentId = Math.floor(Math.random()*7000);
		console.log(JSON.stringify($scope.newComment));
		AppComment.createAppComment.post($scope.newComment, function(newComment) {
